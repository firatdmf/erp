...
class Book(models.Model):
    ...
    # How this book signs the documents it produces — invoices, order
    # confirmations and packing lists all print this, not `name`.
    #
    # They are different things. `name` is the internal handle for the
    # ledger ("DEMFIRAT"), short and unique because people pick a book
    # by it in dropdowns; this is the customer-facing lockup
    # ("DEMFIRAT® | Karven Home Collection"), which is longer, carries
...
class AssetCash(models.Model):
    ...
    # balance in base currency from settings.py (in our case USD)
    balance_base = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True
    )
