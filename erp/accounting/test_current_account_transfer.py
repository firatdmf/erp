...
class TransferTestBase(TestCase):
    def setUp(self):
        ...
        self.elsewhere = CurrentAccount.objects.create(
        ...
        CurrentAccountMovement.objects.create(
            current_account=self.a, book=self.book, date="2026-01-01",
...
class CurrentAccountTransferModelTest(TransferTestBase):
    ...
    def test_posting_moves_the_debt_and_leaves_the_total_alone(self):
        t = CurrentAccountTransfer.objects.create(
            book=self.book, date="2026-02-01", from_current_account=self.a, to_current_account=self.b,
    ...
    def test_posting_twice_writes_one_pair(self):
        ...
        self.assertEqual(CurrentAccountMovement.objects.filter(current_account=self.b).count(), 1)
        self.assertEqual(self.balances(), (Decimal("600.00"), Decimal("400.00")))
    ...
    def test_unposting_leaves_both_statements_as_if_it_never_happened(self):
        ...
        self.assertEqual(CurrentAccountMovement.objects.filter(current_account=self.b).count(), 0)

    def test_an_account_cannot_transfer_to_itself(self):
        with self.assertRaises(ValidationError):
            CurrentAccountTransfer.objects.create(
                book=self.book, date="2026-02-01", from_current_account=self.a, to_current_account=self.a,
                amount=Decimal("400.00"), currency=self.usd,
            )
    ...
    def test_a_transfer_cannot_span_two_books(self):
        with self.assertRaises(ValidationError):
            CurrentAccountTransfer.objects.create(
                ...
                from_current_account=self.a, to_current_account=self.elsewhere,
                amount=Decimal("400.00"), currency=self.usd,
            )
...
class TransferPageModeTest(TransferTestBase):
    ...
    def test_current_account_mode_only_offers_accounts_of_this_book(self):
        response = self.client.get(self.url() + "?mode=current_account")
    ...
    def test_the_page_posts_a_mode_the_view_answers(self):
        """The tests posted "current_account" while the page posted an older
        token, which the view did not recognise and served as cash mode — so
        every virman came back asking for two cash accounts. Read the token
        out of the rendered page rather than restating it here."""
    ...
    def test_posting_current_account_mode_records_who_did_it(self):
    ...
    def test_a_rejected_current_account_transfer_moves_nothing(self):
        response = self.client.post(self.url(), {
            "mode": "current_account",
            "book": self.book.pk,
            "date": "2026-02-01",
            "from_current_account": self.a.pk,
            "to_current_account": self.a.pk,
            "amount": "400.00",
            "currency": self.usd.pk,
        })

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["mode"], "current_account")
        self.assertTrue(response.context["current_account_form"].errors)
        self.assertEqual(self.balances(), (Decimal("1000.00"), Decimal("0.00")))
...
class TransferOnTheStatementTest(TransferTestBase):
    ...
    def rows_for(self, current_account):
        from accounting.views_accounts import _attach_links
        return _attach_links([
            {"mv": mv, "balance_after": Decimal("0")}
            for mv in CurrentAccountMovement.objects.filter(current_account=current_account)
        ])
...
class TransferFormRenderingTest(TransferTestBase):
    ...
    def test_account_options_carry_their_balance(self):
        response = self.client.get(self.url() + "?mode=current_account")
        html = response.context["current_account_form"]["from_current_account"].as_widget()
    ...
    def test_the_page_waits_for_a_rate_before_promising_a_figure(self):
        """A current account balance is carried in base currency. Transfer in
    ...
    def test_currency_options_carry_their_code(self):
        ...
        response = self.client.get(self.url() + "?mode=current_account")
        html = response.context["current_account_form"]["currency"].as_widget()
    ...
    def test_both_panes_render_so_switching_needs_no_round_trip(self):
        ...
        self.assertContains(response, 'id="tfCurrentAccountForm"')
...
class TransferPickerTest(TransferTestBase):
    ...
    def test_the_select_still_carries_the_field_name(self):
        response = self.client.get(self.url() + "?mode=current_account")
        self.assertContains(response, 'name="from_current_account"')
        self.assertContains(response, 'name="to_current_account"')
    ...
    def test_every_account_is_in_the_page_to_search(self):
        ...
        response = self.client.get(self.url() + "?mode=current_account")
        html = response.context["current_account_form"]["from_current_account"].as_widget()
        for current_account in (self.a, self.b):
            self.assertIn(str(current_account.pk), html)
            self.assertIn(current_account.name, html)
    ...
    def test_picking_by_typing_posts_the_same_as_picking_from_the_list(self):
        ...
        response = self.client.post(self.url(), {
            ...
            "book": self.book.pk,
            "date": "2026-02-01",
            "from_current_account": str(self.a.pk),
            "to_current_account": str(self.b.pk),
            "amount": "400.00",
            "currency": self.usd.pk,
        })
        self.assertEqual(response.status_code, 302)
...
class CurrentAccountTransferRateTest(TransferTestBase):
    ...
    def transfer(self, **kwargs):
        ...
        t = CurrentAccountTransfer.objects.create(**defaults)
        t.post()
        return t
    ...
    def test_the_form_refuses_a_non_positive_rate_without_a_500(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn("exchange_rate", response.context["current_account_form"].errors)
        self.assertEqual(CurrentAccountTransfer.objects.count(), 0)
    ...
    def test_the_rate_typed_on_the_form_reaches_the_ledger(self):
        ...
        self.assertEqual(response.status_code, 302)
        t = CurrentAccountTransfer.objects.get()
