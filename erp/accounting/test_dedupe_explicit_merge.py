...
class ExplicitMerge(TestCase):
    def setUp(self):
        ...
        for code, year, amount in (("88888", "2024", "63836.66"),
            ...
            self.accounts[code] = current_account

    def _run(self, **kwargs):
        out = StringIO()
        call_command("dedupe_current_accounts", stdout=out, stderr=out, **kwargs)
        return out.getvalue()
    ...
    def test_the_named_group_merges_into_the_named_survivor(self):
        ...
        self.assertFalse(CurrentAccount.objects.filter(code__in=["88888", "2025X"]).exists())
        survivor = CurrentAccount.objects.get(code="99999")
    ...
    def test_the_survivor_is_the_one_named_not_the_one_ranked(self):
        ...
        self._run(book=self.book.pk, merge="99999,2025X", into="88888",
                  merge_cross_entity=True, apply=True)
        self.assertTrue(CurrentAccount.objects.filter(code="88888").exists())
        self.assertFalse(CurrentAccount.objects.filter(code="99999").exists())

    def test_a_dry_run_changes_nothing(self):
        out = self._run(book=self.book.pk, merge="88888,2025X", into="99999",
                        merge_cross_entity=True)
        self.assertIn("Dry run", out)
        self.assertEqual(CurrentAccount.objects.filter(book=self.book).count(), 3)

    def test_opening_balances_are_carried_not_dropped(self):
        for code, amount in (("88888", "63836.66"), ("2025X", "37926.68"),
                             ("99999", "29979.00")):
            CurrentAccount.objects.filter(code=code).update(opening_balance=Decimal(amount))
        self._run(book=self.book.pk, merge="88888,2025X", into="99999",
                  merge_cross_entity=True, apply=True)
        self.assertEqual(CurrentAccount.objects.get(code="99999").opening_balance,
                         Decimal("131742.34"))
    ...
    def test_merging_across_books_is_refused(self):
        ...
        money between them."""
        CurrentAccount.objects.create(book=self.other_book, code="E-1",
    ...
    def test_an_ambiguous_code_asks_which_book(self):
        CurrentAccount.objects.create(book=self.other_book, code="99999",
    ...
    def test_a_currency_mismatch_still_blocks(self):
        CurrentAccount.objects.filter(code="88888").update(default_currency=self.eur)
