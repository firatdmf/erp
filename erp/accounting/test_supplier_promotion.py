...
class MarkAsSupplierTest(TestCase):
    ...
    def test_a_customer_becomes_both_rather_than_stopping_being_a_customer(self):
        current_account = self._current_account("customer")
        self.assertTrue(mark_as_supplier(current_account))
        current_account.refresh_from_db()
        self.assertEqual(current_account.type, "both")

    def test_an_account_that_is_neither_becomes_a_supplier(self):
        current_account = self._current_account("other")
        self.assertTrue(mark_as_supplier(current_account))
        current_account.refresh_from_db()
        self.assertEqual(current_account.type, "supplier")

    def test_a_supplier_is_left_alone(self):
        current_account = self._current_account("supplier")
        self.assertFalse(mark_as_supplier(current_account))
        current_account.refresh_from_db()
        self.assertEqual(current_account.type, "supplier")

    def test_both_is_left_alone(self):
        current_account = self._current_account("both")
        self.assertFalse(mark_as_supplier(current_account))
        current_account.refresh_from_db()
        self.assertEqual(current_account.type, "both")
    ...
    def test_staff_is_left_alone(self):
        ...
        current_account = self._current_account("staff")
        self.assertFalse(mark_as_supplier(current_account))
        current_account.refresh_from_db()
        self.assertEqual(current_account.type, "staff")
...
class PurchaseInvoicePromotesTheAccountTest(TestCase):
    ...
    def test_issuing_a_purchase_invoice_marks_the_account(self):
        ...
        self.assertEqual(self.current_account.type, "both")
