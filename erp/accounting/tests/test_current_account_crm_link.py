...
CurrentAccountEdit never touched contact/company/supplier, so those accounts had no
...
current account.supplier, empty on every imported account) rather than closing it.

Run with:
    python manage.py test accounting.test_current_account_crm_link
"""
...
class CrmLinkPicker(TestCase):
    ...
    def _link(self, **post):
        return self.client.post(
            reverse("accounts:crm_link", args=[self.current_account.pk]), post, follow=True)

    def _search(self, q):
        r = self.client.get(reverse("accounts:crm_search", args=[self.current_account.pk]), {"q": q})
        self.assertEqual(r.status_code, 200)
        return r.json()["results"]

    # ── the model's own answer ────────────────────────────────────────
    def test_an_unlinked_account_says_so_rather_than_raising(self):
        self.assertIsNone(self.current_account.crm_link)
        self.assertEqual(self.current_account.crm_link_field, "")

    def test_a_linked_account_names_the_one_field_that_holds_it(self):
        self.current_account.company = Company.objects.create(name="Gürhan Tekstil")
        self.assertEqual(self.current_account.crm_link_field, "company")
        self.assertEqual(self.current_account.crm_link.name, "Gürhan Tekstil")
    ...
    def test_attaching_an_existing_record(self):
        ...
        self.assertEqual(self.current_account.company_id, company.pk)
    ...
    def test_re_pointing_a_link_clears_the_old_one(self):
        ...
        self.current_account.refresh_from_db()
        self.assertEqual(self.current_account.company_id, company.pk)
        self.assertIsNone(self.current_account.contact_id)
    ...
    def test_a_record_already_taken_in_this_book_is_refused_by_name(self):
        ...
        holder = CurrentAccount.objects.create(
        ...
        self.current_account.refresh_from_db()
        self.assertIsNone(self.current_account.company_id)
    ...
    def test_the_same_record_may_hold_one_account_per_book(self):
        ...
        CurrentAccount.objects.create(book=self.other_book, code="E-001",
    ...
    def test_creating_carries_what_the_account_already_knows(self):
        ...
        self._link(action="create", kind="company", name=self.current_account.name)
        self.current_account.refresh_from_db()
        company = self.current_account.company
    ...
    def test_creating_a_contact_and_a_supplier_too(self):
        ...
        self.current_account.refresh_from_db()
        self.assertEqual(self.current_account.contact.name, "Gürhan")

        self._link(action="create", kind="supplier", name="Karven Mensucat")
        self.current_account.refresh_from_db()
        self.assertIsNone(self.current_account.contact_id)
        self.assertEqual(self.current_account.supplier.company_name, "Karven Mensucat")
    ...
    def test_detaching_leaves_the_ledger_alone(self):
        ...
        self.current_account.refresh_from_db()
        self.assertIsNone(self.current_account.company_id)
        self.assertEqual(self.current_account.cached_balance, Decimal("608.26"))
    ...
    def test_the_card_is_drawn_on_an_unlinked_account(self):
        ...
        r = self.client.get(reverse("accounts:detail", args=[self.current_account.pk]))
        self.assertEqual(r.status_code, 200)
        # The search URL is only ever emitted by the picker's markup.
        self.assertContains(r, reverse("accounts:crm_search", args=[self.current_account.pk]))
        self.assertNotContains(r, 'value="detach"')

    def test_a_linked_account_shows_the_record_and_a_way_out(self):
        self.current_account.company = Company.objects.create(name="Gürhan Tekstil")
        self.current_account.save(update_fields=["company"])
        r = self.client.get(reverse("accounts:detail", args=[self.current_account.pk]))
        self.assertContains(r, "Gürhan Tekstil")
        self.assertContains(r, 'value="detach"')
        self.assertNotContains(r, reverse("accounts:crm_search", args=[self.current_account.pk]))
    ...
    def test_an_other_account_is_not_warned_about(self):
        ...
        too."""
        self.current_account.type = "other"
        self.current_account.save(update_fields=["type"])
        r = self.client.get(reverse("accounts:detail", args=[self.current_account.pk]))
    ...
    def test_the_retail_counter_is_created_as_other(self):
        ...
        self.assertEqual(get_or_create_retail_current_account(member=self.member).type, "other")
    ...
    def test_search_names_the_account_already_holding_a_candidate(self):
        company = Company.objects.create(name="MARKİSS TEKSTİL")
        CurrentAccount.objects.create(book=self.book, code="00163", name="MARKİSS",
                                   company=company, default_currency=self.usd)
    ...
    def test_a_candidate_taken_in_another_book_is_still_offered(self):
        company = Company.objects.create(name="Karven")
        CurrentAccount.objects.create(book=self.other_book, code="E-001", name="Karven",
                                   company=company, default_currency=self.usd)
...
class CrmLinkFilter(TestCase):
    ...
    def setUp(self):
        ...
        CurrentAccount.objects.create(book=self.book, code="00554", name="Imported",
                                   default_currency=self.usd)
