...
class ObjectPagesAreRefusedAcrossBooks(TestCase):
    def setUp(self):
        ...
        self.their_current_account = self._current_account(self.laleli, "Karven")
        self.my_current_account = self._current_account(self.ergene, "Bursa Tekstil")
        self.their_invoice = self._invoice(self.laleli, self.their_current_account)
        self.their_payment = self._payment(self.laleli, self.their_current_account)
    ...
    def _current_account(self, book, name):
    ...
    def _payment(self, book, current_account):
        return Payment.objects.create(
            book=book, current_account=current_account, number="TAH-001", type="collection",
    ...
    def test_it_is_404_not_403(self):
        ...
        self.client.force_login(self.outsider)
        self.assertEqual(
            self.client.get(self._urls()["account detail"]).status_code, 404)

    def test_their_own_book_is_untouched(self):
        self.client.force_login(self.outsider)
        self.assertEqual(
            self.client.get(reverse("accounts:detail",
                                    args=[self.my_current_account.pk])).status_code, 200)
