...
Check / Promissory Note views (Phase 5).
    ...
    /accounting/accounts/checks/                   → CheckList
    /accounting/accounts/checks/new/               → CheckCreate
    /accounting/accounts/checks/new/?account=<id>  → CheckCreate prefilled
    /accounting/accounts/checks/<id>/              → CheckDetail
    /accounting/accounts/checks/<id>/endorse/      → CheckEndorse  (received only)
    /accounting/accounts/checks/<id>/deposit/      → CheckDeposit  (received only)
    /accounting/accounts/checks/<id>/clear/        → CheckClear
    /accounting/accounts/checks/<id>/bounce/       → CheckBounce   (received only)
    /accounting/accounts/checks/<id>/cancel/       → CheckCancel
    /accounting/accounts/checks/<id>/delete/       → CheckDelete (only if status=cancelled and no movements)
"""
...
class CheckList(View):
    template_name = "accounts/check_list.html"
...
class CheckDetail(View):
    template_name = "accounts/check_detail.html"
