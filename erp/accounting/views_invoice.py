...
Invoice views (Phase 2).
    ...
    /accounting/accounts/invoices/<id>/cancel/       → InvoiceCancel
    /accounting/accounts/invoices/<id>/restore/      → InvoiceRestore
    /accounting/accounts/invoices/<id>/delete/       → InvoiceDelete (draft only)
...
class InvoiceList(View):
    ...
    def get(self, request):
        ...
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return render(request, "accounts/partials/invoice_list_results.html", ctx)
...
class InvoiceEdit(View):
    ...
    def _purchase_redirect(self, request, invoice):
        ...
        return redirect("accounts:purchase_order_detail", pk=invoice.pk)
    ...
    def post(self, request, pk):
        ...
        if not items_locked:
            try:
                ...
                messages.error(request, str(e))
                return redirect("accounts:invoice_edit", pk=pk)
            if not items_data:
                messages.error(request, _g("You must add at least one invoice item."))
                return redirect("accounts:invoice_edit", pk=pk)
