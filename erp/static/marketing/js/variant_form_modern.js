...
function renderVariantImages(variantIndex) {
    ...
    // Build simple HTML - compact 2-column grid layout
    const html = variantImg.images.map((img, idx) => {
        const isPrimary = idx === 0;
        return `
            <div class="variant-sortable-image" data-image-id="${img.id}" style="position: relative; border: 2px solid ${isPrimary ? '#667eea' : '#e5e7eb'}; border-radius: 6px; padding: 3px; background: white; cursor: move;">
                ${isPrimary ? '<div style="position: absolute; top: -5px; left: 2px; background: #667eea; color: white; padding: 1px 3px; border-radius: 6px; font-size: 7px; font-weight: 600; z-index: 1;">PRIMARY</div>' : ''}
                <img src="${img.url}" alt="Variant image" style="width: 100%; height: 45px; object-fit: cover; border-radius: 4px; display: block;">
                <button type="button" class="remove-variant-btn" onclick="event.stopPropagation(); removeVariantImage(${variantIndex}, ${idx})" style="position: absolute; bottom: 0; right: 0; background: #ef4444; color: white; border: none; border-radius: 0 0 4px 0; padding: 2px 4px; font-size: 8px; cursor: pointer; opacity: 0.9;">
...
function prepareVariantsForSubmission() {
    ...
    combinations.forEach((combo, index) => {
        ...
        // Add product attributes for this variant
        const variantProductAttrs = [];
        productAttributes.forEach((attr, attrIdx) => {
            const attrInput = document.querySelector(`input[name="variant_${index + 1}_attr_${attrIdx}"]`);
            if (attrInput && attrInput.value.trim()) {
                variantProductAttrs.push({
                    name: attr.name,
                    value: attrInput.value.trim()
                });
            }
        });
        if (variantProductAttrs.length > 0) {
            variantData.product_attributes = variantProductAttrs;
        }
